...
class Pstudent(Qstudent):
    ...
    def __init__(self, id="x", params={}, knowledge_levels=[0.3, 0.3, 0.2, 0.1, 0.1, 0.1, 0.1], knowledge_names=["KnowMoney", "IntSum", "IntSub", "IntDec", "DecSum", "DecSub", "DecDec"], p_learn_prog=[0.001, 0.001, 0.001, 0.001], threshold=0, nb_try=3):
        # print skills
        Qstudent.__init__(self, knowledge_levels, knowledge_names, threshold=threshold, nb_try=nb_try)
    ...
    def answer(self, exercise, answer=None, nb_try=0):  # act,lvls):
        ...
        prob_P = self.compute_prob_correct_answer(exercise.act)
        Qstudent.learn(exercise.get_knowledges_level(), prob_P)
        prob_correct = prob_P * Qstudent.compute_prob_correct_answer(self, exercise._skills)
